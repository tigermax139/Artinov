function Game(){
	...
	initKey = function(){
		document.onkeydown  = function(e){
			switch(e.keyCode){
				case KEY_UP: 
				if (state != STATE_GAME_GO_DOWN){
					state = STATE_GAME_GO_UP;
				}
					break; 
				case KEY_RIGHT: 
				if (state !=STATE_GAME_GO_LEFT) {
					state = STATE_GAME_GO_RIGHT;
				}
					break;
				case KEY_DOWN: 
				if (state != STATE_GAME_GO_UP) {
					state = STATE_GAME_GO_DOWN;
				}
					break;
				case KEY_LEFT: 
				if (state != STATE_GAME_GO_RIGHT) {
	...
	this.update = function(){
		...
		if(result){
			...
			if(score%10 == 0){
				_score.setAttribute('value', score+' Good Job, my Friend!');
			}else{
				_score.setAttribute('value', score);
			}
		}
		...
		function kill(){
			var xx = snake[0].getX();
			var yy = snake[0].getY();
			for (var i = 1; i < snake.length;) {
				var zx = snake[i].getX();
				var zy = snake[i].getY();
				if ( (xx==zx)&&(yy==zy) ){
					var end = confirm("Game OVER, your score = "+ score + "  Do you want start new game??");
					if (end == true) {
